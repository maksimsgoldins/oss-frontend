  ...
  useEffect(() => {
    ...
    if (selected && !isEditing && !isNew) {
      setForm({
        ...
        possibleValuesText: (selected.possible_values || []).join(", "),
  ...
  async function save(e) {
    ...
    try {
      await api.createAttribute({
        ...
        possible_values: form.possibleValuesText
          .split(",")
          .map(x => x.trim())
          .filter(Boolean)
      });
    ...
    <>
      <div className="panel">
        <div className="header-line">
          <div>
            <h2 style={{ marginTop: 0, marginBottom: 4 }}>Attributes</h2>
            <div className="muted">Supports possible values and list type.</div>
          </div>
        ...
        <div className="panel">
          ...
          {items.map(item => (
            <button key={item.id} className={`list-button${item.id === selectedId ? " active" : ""}`} onClick={() => selectItem(item)}>
              ...
              <span className="muted">{item.code} · {item.value_type}</span>
            </button>
          ))}
          {!items.length && <div className="muted">No attributes yet.</div>}
        </div>

        <div className="panel">
          <div className="header-line">
            <div>
              <h2 style={{ marginTop: 0, marginBottom: 4 }}>Attribute Details</h2>
          ...
          <form onSubmit={save} className={isEditing ? "" : "readonly"}>
            ...
            <div className="field">
              ...
              <input value={form.possibleValuesText} onChange={e => setForm({ ...form, possibleValuesText: e.target.value })} />
            </div>
            <div className="field">
          ...
          {!isEditing && possibleValues.length > 0 && (
            <div style={{ marginTop: 12 }}>
              {possibleValues.map(x => <span key={x} className="pill">{x}</span>)}
            </div>
          )}
