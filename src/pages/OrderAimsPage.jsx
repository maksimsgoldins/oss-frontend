...
const emptyForm = { code:"", name:"", subAimsText:"" };
...
export default function OrderAimsPage() {
  ...
  useEffect(() => {
    ...
    if (selected && !isEditing && !isNew) {
      setForm({ code:selected.code, name:selected.name, subAimsText:(selected.sub_aims || []).map(x => x.code).join(", ") });
    }
  }, [items, selectedId, isEditing, isNew]);

  function newItem() { setSelectedId(null); setForm(emptyForm); setIsEditing(true); setIsNew(true); }
    ...
    <>
      <div className="panel">
        <div className="header-line">
          <div><h2 style={{marginTop:0, marginBottom:4}}>Order Aims</h2><div className="muted">General classifier.</div></div>
      ...
      <div className="two-col">
        <div className="panel">
          ...
          {items.map(item => (
            <button key={item.id} className={`list-button${item.id === selectedId ? " active" : ""}`} onClick={() => selectItem(item)}>
              <strong>{item.name}</strong><br /><span className="muted">{item.code}</span>
            </button>
          ))}
        </div>
        ...
        <div className="panel">
          <div className="header-line">
            <div><h2 style={{marginTop:0, marginBottom:4}}>Order Aim Details</h2><div className="muted">{isEditing ? "Edit mode" : "Read-only mode"}</div></div>
            <div className="row">
              {!isEditing && !isNew && selectedId && <button className="btn secondary" onClick={() => setIsEditing(true)}>Edit</button>}
          ...
          {!isEditing && form.subAimsText && <div style={{marginTop:12}}>{form.subAimsText.split(",").map(x => x.trim()).filter(Boolean).map(x => <span key={x} className="pill">{x}</span>)}</div>}
